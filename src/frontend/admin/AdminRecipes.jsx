...
const AdminRecipes = () => {
  ...
  const handleFeatureStatusChange = async (recipeId, status) => {
    try {
      setActionLoading(prev => ({
        ...prev,
        [`feature-${recipeId}`]: true
      }));

      console.log("recipeId", recipeId + "status" + status);

      await axios.post(
        "http://localhost:8080/api/admin/recipe/feature",
        null,
        {
          params: { recipeId: recipeId, status: status },
          headers: {
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <select
                          onChange={(e) => {
                            ...
                            handleFeatureStatusChange(recipe.id, newValue === "feature");
                          }}
                          ...
                          className="px-3 py-2 rounded-lg border border-gray-300 bg-white text-sm cursor-pointer hover:border-yellow-500 focus:outline-none focus:ring-2 focus:ring-yellow-500 disabled:opacity-50 disabled:cursor-not-allowed transition duration-150"
                        >
                          {actionLoading[`feature-${recipe.id}`] ? (
                            ...
                            <>
                              ...
                              <option value="unfeature">Not Featured</option>
                            </>
                          )}
                        </select>
